...
public class MonsterEffect {
    ...
    public static void terratiger(String onlineUser) {
        do {
            ...
            response = GameMatView.getCommand();
            if (response.equals("cancel")) {
                GameMatView.showInput("Your Monster Effect is not activated!");
                return;
            }
        } while (!response.matches("yes|no"));
        if (response.equals("no")) {
            ...
            else if (MonsterZoneCard.getNumberOfFullHouse(onlineUser) == 5)
                GameMatView.showInput("Oops! You cant have Special Summon because of no free space in Monster Zone!");
            else {
                int address;
                String cardName;
                while (true) {
